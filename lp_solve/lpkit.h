...
#define CALLOC(ptr, nr)\
  if(!(ptr = calloc((size_t)(nr), sizeof(*ptr))) && nr) {\
    fprintf(stderr, "calloc of %" PRIuMAX " bytes failed on line %d of file %s\n",\
            (uintmax_t)(nr * sizeof(*ptr)), __LINE__, __FILE__);\
    exit(EXIT_FAILURE);\
  }

#define MALLOC(ptr, nr)\
  if(!(ptr = malloc((size_t)((nr) * sizeof(*ptr)))) && nr) {\
    fprintf(stderr, "malloc of %" PRIuMAX " bytes failed on line %d of file %s\n",\
            (uintmax_t)(nr * sizeof(*ptr)), __LINE__, __FILE__);\
    exit(EXIT_FAILURE);\
  }
...
#define REALLOC(ptr, nr)\
  if(!(ptr = realloc(ptr, (size_t)((nr) * sizeof(*ptr)))) && nr) {\
    fprintf(stderr, "realloc of %" PRIuMAX " bytes failed on line %d of file %s\n",\
            (uintmax_t)(nr * sizeof(*ptr)), __LINE__, __FILE__);\
    exit(EXIT_FAILURE);\
  }
