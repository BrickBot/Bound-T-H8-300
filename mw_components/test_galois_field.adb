...
procedure TEST_GALOIS_FIELD is
...
begin
  declare
    ...
    package GALOIS_FIELD_P_N is new GALOIS_FIELD(P,N);
  ...
  begin
    ...
    begin
      loop
        NEW_LINE;
        A := TO_ELEMENT(GET_POLYNOMIAL("ENTER ELEMENT TO INVERSE"));
        exit when A = ZERO;
        PUT("INVERSE OF ");
        PUT(TO_POLYNOMIAL(A));
        PUT(" : ");
    ...
    begin
      loop
        NEW_LINE;
        A := TO_ELEMENT(GET_POLYNOMIAL("ENTER ELEMENT"));
    ...
    declare
      ...
      GEN : constant ELEMENT_ARRAY := ALL_GENERATORS;
